...
prompt_variable "KIT_DB_HOST" "$current_shell"
prompt_variable "KIT_DB_USER" "$current_shell"
prompt_variable "KIT_DB_PASSWORD" "$current_shell"
prompt_variable "KIT_DB_NAME" "$current_shell"
...
while true; do
    ...
    case $OS_ARCH in
        1)
            # macOS (Intel)
            BINARY_PATH="amd/kitutil"
            break
            ;;
        2)
            # macOS (Apple Silicon)
            BINARY_PATH="arm/kitutil"
            break
            ;;
        3)
            # Linux
            BINARY_PATH="linux/kitutil"
            break
            ;;
        *)
...
GITHUB_DOWNLOAD_PATH="https://raw.githubusercontent.com/cpwingert/kitutil-deploy/main/bin"
...
## to debug change to :  log_curl -L -o "$INSTALL_PATH/kitutil" $p
if ! curl -L -o "$INSTALL_PATH/kitutil" $p; then
    echo "Error: Failed to download the binary. Installation aborted."
    exit 1
fi

# Change the permissions of the binary to executable
chmod +x "$INSTALL_PATH/kitutil"
...
echo "Installation completed successfully! Run kitutil to launch application"
